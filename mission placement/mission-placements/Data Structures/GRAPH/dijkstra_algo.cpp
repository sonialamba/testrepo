...
void displaylist(struct graph *g)
{
     ...
     while( v!= NULL )
     {
            printf("v = %d pathlength %d ", v->d , v->pathlength);
...
int printtree(struct graph *g,int start )
{
...
int dijkstra(struct graph *g , int start )
{
    ...
    {       
            ...
            struct arc *pathloc=NULL;                          
            v=g->first;
            while( v != NULL )
            {
                   if( v->intree == true && v->outdegree > 0 )
                   {
                       struct arc *minarcloc=v->firstarc;    
                       struct arc *varcs=v->firstarc;
                       int minpath=v->pathlength;
                       int minweight=32767;
                       ...
                       if( minweight + minpath < newpathlength )
                       {
                           newpathlength = minweight + minpath ;
                           pathloc = minarcloc ;
                       }
                   }
                 v=v->next;
            ...
            {
                pathloc->intree = 1;
                pathloc->destv->intree = 1;
                pathloc->destv->pathlength=newpathlength ;
                count++ ;
            }
